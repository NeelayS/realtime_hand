...
def PSPNet_res101(in_channels=1, n_classes=3):
    model = ResNet(
        Bottleneck, [3, 4, 23, 3], in_channels=in_channels, n_classes=n_classes
    )
    return model


@SEG_MODELS_REGISTRY.register()
def PSPNet_res50(in_channels=1, n_classes=3):
    model = ResNet(
        Bottleneck, [3, 4, 6, 3], in_channels=in_channels, n_classes=n_classes
    )
    return model


def PSPHead_res50(in_channels=1):
    model = PSPHead(Bottleneck, [3, 4, 6, 3], in_channels=in_channels)
    return model
